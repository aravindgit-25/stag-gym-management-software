...
export class SubscriptionComponent implements OnInit {
  ...
  displayList = computed(() => {
    ...
    list.forEach((sub) => {
      ...
      if (
        !existing ||
        sDate > existing.dateTime ||
        (sDate === existing.dateTime && sId > existing.id)
      ) {
    ...
    return Array.from(latestMap.values())
      .map((item) => {
        ...
        const hasPayment = payments.some(
          (p) => Number(p.subscriptionId || (p as any).subscription_id) === Number(sId),
        );
        ...
        return {
          id: sId,
          memberName:
            this.members().find((m) => m.id === Number(sMemberId))?.name || `Member ${sMemberId}`,
  ...
  openViewModal(sub: any): void {
    ...
    const planIds =
      typeof sub.planId === 'string' ? sub.planId.split(',').map(Number) : [Number(sub.planId)];
    ...
    this.paymentService.getPayments().subscribe({
      next: (payments) => {
        ...
        const payment = payments.find(
          (p) => Number(p.subscriptionId || (p as any).subscription_id) === Number(sub.id),
        );
  ...
  openCompletePaymentPopup(details: any): void {
    ...
    const totalAmount =
      (details.plans as Plan[])?.reduce((sum, p) => sum + p.price, 0) || details.plan?.price || 0;
  ...
  async onSaveRenewal() {
    ...
    if (this.renewalForm.valid) {
      const confirmed = await this.confirm.ask(
        'Are you sure you want to renew this membership and record payment?',
      );
      if (!confirmed) return;
